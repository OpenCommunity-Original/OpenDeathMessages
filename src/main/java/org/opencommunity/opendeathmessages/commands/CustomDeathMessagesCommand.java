package org.opencommunity.opendeathmessages.commands;
...
public class CustomDeathMessagesCommand implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (sender.hasPermission("odm.reload")) {
            if (args.length == 1) {
                if (args[0].equalsIgnoreCase("reload")) {
                    ...
                    sender.sendMessage(ChatColor.RED + "To reload: /odm reload");
                    sender.sendMessage(ChatColor.RED + "To add messages to config: /odm add [path] [message]");
                    sender.sendMessage(ChatColor.RED + "To list messages: /odm list [path]");
                    sender.sendMessage(ChatColor.RED + "To remove messages: /odm remove [path] [number]");
                }
            } else if (args.length >= 3) {
                if (sender.hasPermission("odm.add.message")) {
                    if (args[0].equalsIgnoreCase("add")) {
                        ...
                        if (plugin.getConfig().getString(path) != null) {
                            ...
                            sender.sendMessage(ChatColor.translateAlternateColorCodes('&', "&cThis is an invalid path!"));
                        ...
                        if (sender.hasPermission("odm.remove.message")) {
                            String path = args[1];
                                ...
                                if (plugin.getConfig().getString(path) != null) {
                                    ...
                                    try {
                                        ...
                                        if (messages.size() > 1) {
                                            ...
                                            sender.sendMessage(ChatColor.translateAlternateColorCodes('&', "&cYou must add a message before you can remove the last one!"));
                                    } catch (IndexOutOfBoundsException e) {
                                        sender.sendMessage(ChatColor.translateAlternateColorCodes('&', "&cThis is an invalid number! Options are " + list));
                                    }
                                } else
                                    sender.sendMessage(ChatColor.translateAlternateColorCodes('&', "&cThis is an invalid path!"));
                ...
                if (sender.hasPermission("odm.list")) {
                ...
                if (sender.hasPermission("odm.toggle")) {
